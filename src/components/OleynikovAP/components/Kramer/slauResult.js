...
const slauResult = (matrix) => {
  if (Array.isArray(matrix)) {
    ...
    for (let i = 0; i < matrix.length; i++) {
      ...
      for (let j = 0; j < matrix.length + 1; j++) {
        if (typeof (Number.parseFloat(matrix[i][j])) !== 'number' || Number.isNaN(matrix[i][j])) { throw new Error('неверный формат данных в матрице'); }
  ...
  const roots = new Array(matrix.length);
  if (matrix.length === 1) {
    if (matrix[0][0] !== 0) {
  ...
  const d0 = compDeterm(formMatrix(matrix, 0));
  if (!d0) {
    return { roots: null };
  }
  for (let j = 0; j < matrix.length; j++) {
